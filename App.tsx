...
import { compressImage, checkBackendHealth } from './services/api';
...
export default function App() {
  ...
  const handleFileSelect = (selectedFile: File) => {
    ...
    // Determine simple maxK estimate based on file size (very rough heuristic)
    // In a real app, backend would return image dimensions to set maxK = min(width, height)
    setMaxK(200); // Default max
    setKValue(50); // Default start
  };

  // Trigger compression when k changes or file changes
  useEffect(() => {
    ...
    const process = async () => {
      ...
      setError(null);
      try {
        const response = await compressImage(file, debouncedK);
